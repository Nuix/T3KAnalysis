...
public class ObjectDetectionTests {
    static class ImageResultMock extends ImageResult {
        ...
        public ImageResultMock() {
            getDetections().add(new ImageResultMock.ObjectDetectionMock());
        }
    ...
    public void metadataIsAssigned() {
        ...
        String objectDetectionMD = String.format("%s|%d|%s|Score", T3KMetadata.T3K_DETECTION, 1, "gun");
        assertTrue(metadataMap.containsKey(objectDetectionMD));
        assertEquals(78.0, ((Number)metadataMap.get(objectDetectionMD)).doubleValue(), 0.2);
    }
